...
def state():
    ...
    if not cents:
        # Fallback al float en euros si no hay céntimos
        try:
...
def place():
    ...
    for b in bets:
        ...
        if amount < min_cell:
            return jsonify({"ok": False, "error": f"Apuesta mínima por casilla: {min_cell}¢"}), 400
        total += amount
    ...
    import time
    return jsonify({"ok": True, "session_id": int(time.time()*1000), "balance": current_user.saldo_centimos})
...
def spin():
    ...
    bets = data.get("bets") or []
    if not bets:
        return jsonify({"ok": False, "error": "Faltan apuestas (bets) para liquidar"}), 400

    client_result = (request.get_json(force=True) or {}).get("result", None)
    try:
...
def _sync_balance_from_cents(user):
    try: user.balance = (user.saldo_centimos or 0) / 100.0
    except: user.balance = 0.0
